        ...
        for (const size in cartItems[productId]) {
          if (cartItems[productId][size] > 0) {
            orderItems.push({
              ...
              image:
                products.find((p) => p._id === productId)?.image?.[0] || "",
            });
          }
        }
      }
        ...
        for (const size in cartItems[productId]) {
          ...
          if (quantity > 0) {
            ...
            if (discountInfo && quantity >= discountInfo.minQty) {
              ...
              const itemDiscount =
                (discountedQty * mrp * discountInfo.percent) / 100;
        ...
        <div className="mt-8 min-w-80">
          <div className="bg-white rounded-lg shadow p-5 mb-6">
                  ...
                  return (
                    ...
                    >
                      ...
                      {(() => {
                        // Parse discount note
                        const note =
                          priceObj.discountNote || product.discountNote;
                        const discountInfo = (() => {
                          const regex = /Buy\s*(\d+):?\s*Get Flat (\d+)% off/i;
                          const match = note && note.match(regex);
                          if (match) {
                        ...
                        if (discountInfo && quantity >= discountInfo.minQty) {
                          const eligibleSets = Math.floor(
                            quantity / discountInfo.minQty
                          );
                          const discountedQty =
                            eligibleSets * discountInfo.minQty;
                          const itemDiscount =
                            (discountedQty *
                              priceObj.mrp *
                              discountInfo.percent) /
                            100;
                          itemTotal = priceObj.mrp * quantity - itemDiscount;
                        } else {
                          itemTotal =
            ...
            {(() => {
              ...
              let discountTotal = 0;
              let totalMrp = 0;
              const appliedDiscountNotes = new Set();
                ...
                for (const size in cartItems[productId]) {
                  ...
                  if (quantity > 0) {
                    ...
                    if (discountInfo && quantity >= discountInfo.minQty) {
                      ...
                      // Add discounted amount to subtotal
                      subtotal += mrp * quantity - itemDiscount;
                      if (note && !appliedDiscountNotes.has(note)) {
                        appliedDiscountNotes.add(note);
                      }
                    } else {
                      // If no discount, use offer price
                      subtotal += offer * quantity;
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span>
                      {Object.entries(cartItems).reduce(
                        (acc, [_, sizes]) =>
                          acc + Object.values(sizes).reduce((a, b) => a + b, 0),
                        0
                      )}
                    </span>
                  </div>
                      ...
                      <div className="flex justify-between text-[--primary-color] text-sm">
                        ...
                        <div className="flex flex-col text-xs text-[--primary-color]">
                          {Array.from(appliedDiscountNotes).length > 0 && (
                            <span>
                              {" "}
                              {Array.from(appliedDiscountNotes).join(", ")}
                            </span>
                          )}
                        </div>
