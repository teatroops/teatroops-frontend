...
const Login = () => {
    ...
    try {
      if (currentState === 'Sign Up') {
        ...
        if (response.data.success) {
          ...
          try {
            const profileRes = await axios.get(backendUrl + '/api/user/profile', { headers: { token: response.data.token } });
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800'>
      ...
      <button className='bg-[--primary-color] border-2 border-green-700 text-white font-light px-8 py-2 mt-4'>{currentState === 'Login' ? 'Sign In' : 'Sign Up'}</button>
    </form>
  )
}
